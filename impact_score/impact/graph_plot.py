...
class GraphPlotter:
    ...
    def get_kill_dataframe(self) -> pd.DataFrame:
        ...
        # Create a column called means, which has weapon_name or AbilityName if its not 0
    ...
    def plot_round(self, **kwargs):
        ...
        plt.plot(self.plot_data["Round time"], self.plot_data["Win_probability"],
                 color=color_dict[chosen_side], zorder=0)
        self.ax = plt.gca()
    ...
    def apply_markers(self):
        ...
        for round_time, win_probability, color in zip(x_data, y_data, color_data):
            if color is not None:
                self.ax.scatter(x=round_time, y=win_probability, color=color, edgecolor="w", marker="o", s=70, zorder=2)
