...
def prepare_dataset(filename: str = "4000.csv") -> pd.DataFrame:
    ...
    redundant_features_atk = [f"ATK_{item}" for item in ["weaponValue", "shields", "remainingCreds"]]
    redundant_features_def = [f"DEF_{item}" for item in ["weaponValue", "shields", "remainingCreds"]]
